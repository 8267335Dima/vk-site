...
async def _execute_task_logic(ctx, task_history_id: int):
    ...
    async with AsyncSessionFactory() as session:
        task_history = await session.get(TaskHistory, task_history_id, options=[
        ...
        # Используем готовый пул Redis из контекста воркера
        redis_pool = ctx['redis_pool']
        emitter = RedisEventEmitter(redis_pool)
        emitter.set_context(task_history.user.id, task_history_id)
        
        try:
            task_key_str = next((item.id for item in AUTOMATIONS_CONFIG if item.name == task_history.task_name), None)
            if not task_key_str:
                raise ValueError(f"Не удалось найти ключ задачи для имени '{task_history.task_name}'")

            ServiceClass, method_name, ParamsModel = TASK_CONFIG_MAP[TaskKey(task_key_str)]
            params = ParamsModel(**(task_history.parameters or {}))
            ...
            service_instance = ServiceClass(db=session, user=task_history.user, emitter=emitter)
            ...
            log.error("task_runner.auth_error_critical", user_id=task_history.user.id, error=str(e))
            ...
            # Пул больше не закрываем здесь, им управляет воркер
...
async def publish_scheduled_post_task(ctx, post_id: int):
    async with AsyncSessionFactory() as session:
        ...
        try:
            ...
            # Убедитесь, что метод wall_post существует в вашем VKAPI
...
async def run_scenario_from_scheduler_task(ctx, scenario_id: int, user_id: int):
    ...
    # Блокировка в ARQ делается иначе, но для простоты пока опустим
    # В ARQ задачи с одинаковыми job_id не будут дублироваться, если это настроить
    try:
        async with AsyncSessionFactory() as session:
