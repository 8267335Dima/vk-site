...
async def test_user_deletion_cascades(db_session: AsyncSession):
    ...
    # Убедимся, что все создано
    assert (await db_session.execute(select(func.count(TaskHistory.id)))).scalar_one() == 1
    assert (await db_session.execute(select(func.count(Automation.id)))).scalar_one() == 1
    assert (await db_session.execute(select(func.count(Scenario.id)))).scalar_one() == 1
